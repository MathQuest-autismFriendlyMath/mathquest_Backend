...
const interactionEventSchema = new mongoose.Schema({
  ...
  moduleName: {
    type: String,
    required: true,
    enum: ['NumberRecognition', 'Counting', 'Addition', 'Subtraction', 'PatternSequence', 'VisualMultiplication', 'MoneyMath', 'TimeTelling', 'SchedulePlanning', 'WordProblems']
  },
  questionId: {
    type: String,
    required: true
...
interactionEventSchema.statics.getUserPatterns = async function(userId, moduleName, limit = 100) {
  return this.find({ userId, moduleName })
    .sort({ timestamp: -1 })
    .limit(limit);
};

// Static method to calculate engagement metrics
interactionEventSchema.statics.calculateEngagementMetrics = async function(userId, sessionId) {
  ...
  events.forEach(event => {
    ...
    if (event.eventType === 'key_down' || event.eventType === 'key_up') metrics.keyboardInteractionCount++;
    ...
    if (event.eventType === 'choice_hover_start' && event.eventData?.choiceIndex !== undefined) {
    ...
    if (event.eventType === 'choice_hover_end' && event.eventData?.choiceIndex !== undefined) {
      const choiceKey = `choice_${event.eventData.choiceIndex}`;
      if (hoveredChoices.has(choiceKey) && hoveredChoices.get(choiceKey).startTime) {
        const duration = event.timestamp - hoveredChoices.get(choiceKey).startTime;
  ...
  if (reactionTimes.length > 0) {
    metrics.averageReactionTime = reactionTimes.reduce((a, b) => a + b, 0) / reactionTimes.length;
  }
  ...
  hoveredChoices.forEach((value, key) => {
    metrics.hoverPatterns[key] = {
      ...
      averageHoverDuration: value.count > 0 ? value.totalDuration / value.count : 0
    };
  });

  // Calculate total session duration
  if (events.length > 1) {
    metrics.totalDuration = events[events.length - 1].timestamp - events[0].timestamp;
  }
